...
class WaypointParser(Node):
    def __init__(self):
        ...
        with open(csv_path, mode='r', encoding='utf-8') as file:
            reader = csv.reader(file)  # 헤더를 무시하고 CSV를 읽기
            self.point_data = []
            next(reader)  # 첫 번째 줄(헤더)은 건너뜁니다
            for row in reader:
                index, longitude, latitude = row
                self.point_data.append((int(index), float(longitude), float(latitude)))
...
def main(args=None):
    ...
    waypoint_parser = WaypointParser()  # 노드 생성
    try:
        rclpy.spin(waypoint_parser)  # 노드 실행
    except KeyboardInterrupt:
        pass  # 키보드 인터럽트(CTRL+C)가 들어오면 예외 처리
    finally:
        waypoint_parser.destroy_node()  # 노드 종료
